    ...
    for resource, typ in RESOURCES.items():
        for category in ("hard", "used"):
            ...
            yield {
                ...
                "included": False,
                "name": f"{metricPrefix}.{resource}.{category}",
                "type": "gauge",
            }

METRICS += build_metrics("openshift.clusterquota", "across all namespaces")
METRICS += build_metrics("openshift.appliedclusterquota", "by namespace")
